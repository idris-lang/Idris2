...
function support_system_file_seekLine(file_ptr) {
  ...
  while (lineEnd === -1) {
    const bytesRead = support_system_file_fs.readSync(
      file_ptr.fd,
      readBuf,
      0,
      1,
    ...
    file_ptr.buffer = Buffer.concat([
      file_ptr.buffer,
      readBuf.slice(0, bytesRead),
    ]);
...
function support_system_file_getStr() {
  return support_system_file_readLine({
    fd: 0,
...
function support_system_file_openFile(n, m) {
  try {
    const fd = support_system_file_fs.openSync(
      n,
      support_system_file_parseMode(m),
    );
...
function support_system_file_popen(cmd, m) {
  ...
  if (mode != "r") {
    process.__lasterr =
      "The NodeJS popen FFI only supports opening for reading currently.";
    return null;
  }
  ...
  const write_fd = support_system_file_fs.openSync(tmp_file, "w");
  ...
  switch (mode) {
    ...
    default:
      process.__lasterr =
        "The popen function cannot be used for reading and writing simultaneously.";
      return null;
  }
  ...
  const read_ptr = support_system_file_openFile(tmp_file, "r");
