...
processTTImpDecls {vars} nest env decls
  ...
  where
    ...
    bindNames (IClaim dat@(MkWithData fc (MkIClaimData c vis opts ty)))
        = do ty' <- bindTypeNames dat.fc [] (fromList vars) ty.val
