...
checkClause {vars} mult vis totreq hashit n opts nest env
    ...
    = do (lhs, (vars'  ** (sub', env', nest', lhspat, reqty))) <-
         ...
         wtyScope <- replace rdefs scenv !(nf rdefs scenv (weakensN swargs wval))
                            var
                            !(nf rdefs scenv
                                 (weakensN swargs notreqty))
...
mkRunTime fc n
    = do logC "compile.casetree" 5 $
           do pure $ "Making run time definition for " ++ show !(toFullNames n)
         defs <- get Ctxt
         Just gdef <- lookupCtxtExact n (gamma defs)
              | _ => pure ()
