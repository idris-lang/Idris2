...
bindNotReq fc i {vs = _ :< n} (env :< b) (Keep p) ns tm
   = let tmptm = subst (Ref fc Bound (MN "arg" i)) tm
         (ns', btm) = bindNotReq fc (1 + i) env p ns tmptm in
         (ns', refToLocal (MN "arg" i) n btm)
bindNotReq fc i {vs = _ :< n} (env :< b) (Drop p) ns tm
...
extendUsed : ArgUsed -> (new : Local) -> Usage vars -> Usage (vars <>< new)
extendUsed a [] x = x
extendUsed a (y :: xs) x = extendUsed a xs (x :< a)

dropUsed : (new : Local) -> Usage (vars <>< new) -> Usage vars
dropUsed [] x = x
dropUsed (_ :: xs) (us) = tail (dropUsed xs us)

inExtended : ArgUsed -> (new : Local) ->
             {auto u : Ref Used (Usage vars)} ->
