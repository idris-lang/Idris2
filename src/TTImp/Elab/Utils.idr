...
extendUsed : ArgUsed -> (new : List Name) -> Usage vars -> Usage (Scope.ext vars new)
extendUsed a [] x = x
extendUsed a (y :: xs) x = extendUsed a xs (x :< a)

dropUsed : (new : List Name) -> Usage (Scope.ext vars new) -> Usage vars
dropUsed [] x = x
dropUsed (_ :: xs) (us) = Lib.tail (dropUsed xs us)
