...
changeVar (MkVar {varIdx=x} old) (MkVar new) (Local fc r idx p)
    = if x == idx
...
-- TODO: refactor toRig1, toRig0 as
-- updateAt : (Term vs -> Term vs) -> Var vs -> Env Term vs -> Env Term vs
toRig1 : {idx : Nat} -> (0 p : IsVar nm idx vs) -> Env Term vs -> Env Term vs
toRig1 First (env :< b)
    = if isErased (multiplicity b)
         then env :< setMultiplicity b linear
         else env :< b
toRig1 (Later p) (env :< b) = toRig1 p env :< b

toRig0 : {idx : Nat} -> (0 p : IsVar nm idx vs) -> Env Term vs -> Env Term vs
toRig0 First (env :< b) = env :< setMultiplicity b erased
toRig0 (Later p) (env :< b) = toRig0 p env :< b
...
isNeeded x [] = False
isNeeded x (MkVar {varIdx = i} _ :: xs) = x == i || isNeeded x xs
