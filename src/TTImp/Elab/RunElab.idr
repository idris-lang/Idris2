...
elabScript rig fc nest env script@(NDCon nfc nm t ar args) exp
  ...
  where
    ...
    {-
    To enable automatic derivation of decidable equality:
    For a given type constructor, 
        get all pairs of data constructors that need a decEq clause, 
        i.e. constructors that have the same type 
     -}
    elabCon defs "GetDecEqConPairs" [_, x1]
        ...
        -- we want to be sure a type constructor was passed in
            ...
            let gdefs : List GlobalDef
                gdefs = catMaybes gdefs'
            -- traverse_ (logTerm "auto" 1 "con: ") (map (.type) gdefs)
            let splitTys : (v : Scope) -> Term v -> Core (v' ** Term v')
                splitTys v (Bind _ x (Pi _ _ info ty) sc) = ?sdfj
                splitTys _ _ = ?wesdjfk
                -- for the type of a constructor, go under all binders and get the final type as a list of applied terms in an extended scope
                getLastTy : (v : Scope) -> Term v -> (v' ** List (Term v'))
                ...
                cterms : List (String, (v ** List (Term v)))
                cterms = map (\x => (show x.fullname, getLastTy [] x.type)) gdefs
                ...
                pairs : List ((String, (v ** List (Term v))), (String , (v' ** List (Term v'))))
                pairs = nubBy (\x, y => (fst (fst x) == fst (fst y)) && (fst (snd x) == fst (snd y))) [(x, y) | x <- cterms, y <- cterms]
                -- vals : List (List (String, String)) 
            vals <- traverse (\((x1, (vy1 ** y1)), (x2, (vy2 ** y2))) =>
