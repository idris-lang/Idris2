...
elabScript rig fc nest env script@(NDCon nfc nm t ar args) exp
  ...
  where
    ...
    elabCon : Defs -> String -> Spine vars -> Core (NF vars)
    elabCon defs "Pure" [<_,(_, val)]
        = do empty <- clearDefs defs
             evalClosure empty val
    elabCon defs "Bind" [<_,_,(fcact,act),(_,k)]
        -- act : Elab A
        -- k : A -> Elab B
    ...
    elabCon defs "LogMsg" [<(_,topic), (_,verb), (_,str)]
        = do topic' <- evalClosure defs topic
             ...
             unverifiedLogC !(reify defs topic') !(reify defs verb') $
                  do str' <- evalClosure defs str
                     reify defs str'
             scriptRet ()
    elabCon defs "LogTerm" [<(_,topic), (_,verb), (_,str), (_,tm)]
        = do topic' <- evalClosure defs topic
             verb' <- evalClosure defs verb
             unverifiedLogC !(reify defs topic') !(reify defs verb') $
                  do str' <- evalClosure defs str
             ...
             scriptRet ()
    elabCon defs "LogSugaredTerm" [<(_,topic), (_,verb), (_,str), (_,tm)]
        = do topic' <- evalClosure defs topic
             verb' <- evalClosure defs verb
             unverifiedLogC !(reify defs topic') !(reify defs verb') $
                  do str' <- evalClosure defs str
             ...
             scriptRet ()
    elabCon defs "Check" [<(_,exp), (_,ttimp)]
    ...
    elabCon defs "GetInfo" [<(_,n)]
        = do n' <- evalClosure defs n
             res <- lookupNameInfo !(reify defs n') (gamma defs)
             scriptRet res
    elabCon defs "GetLocalType" [<(_,n)]
        = do n' <- evalClosure defs n
             n <- reify defs n'
    ...
    elabCon defs "GetCons" [<(_,n)]
        = do n' <- evalClosure defs n
             ...
             scriptRet cons
    elabCon defs "GetReferredFns" [<(_,n)]
    ...
    elabCon defs "WriteFile" [<(_,lk), (_,pth), (_,contents)]
        = do pathPrefix <- lookupDir defs !(evalClosure defs lk)
             path <- reify defs !(evalClosure defs pth)
             validatePath defs path
