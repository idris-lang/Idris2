...
-- TODO: refactor
-- E.g. it's very similar to a `Subst` type defined in Compiler.Opts
-- or Core.TT.Subst' Subst
data EEnv : Scope -> Scoped where
     Lin : EEnv free [<]
...
mutual
  ...
  tryApply {free} {vars} rec stk env (MkFun args exp)
      = do let Just (env', stk') = takeFromStack env stk args
           ...
           res <- eval rec env' stk' (embed $ embed exp) -- embed is free!
           pure (Just res)
...
getLams : {done : _} ->
          Int -> CSubst done args -> CExp (args ++ done) ->
          (args' ** (CSubst args' args, CExp (args ++ args')))
getLams {done} i env (CLam fc x sc)
    = getLams {done = done :< x} (i + 1) (env :< CRef fc (MN "ext" i)) sc
getLams i env sc = (done ** (env, sc))
