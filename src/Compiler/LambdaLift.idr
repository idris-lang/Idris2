...
mutual
  ...
  data Lifted : Scoped where
       ...
       LAppName : FC -> (lazy : Maybe LazyReason) -> (n : Name) ->
                  (args : ScopedList (Lifted vars)) -> Lifted vars
       ...
       LConCase : FC -> (expr : Lifted vars) ->
                  (alts : ScopedList (LiftedConAlt vars)) ->
       ...
       LConstCase : FC -> (expr : Lifted vars) ->
                    (alts : ScopedList (LiftedConstAlt vars)) ->
                    (def : Maybe (Lifted vars)) -> Lifted vars
...
data LiftedDef : Type where
     ...
     MkLError : (expl : Lifted SLNil) -> LiftedDef
...
unload : FC -> (lazy : Maybe LazyReason) -> Lifted vars -> ScopedList (Lifted vars) -> Core (Lifted vars)
unload fc _ f SLNil = pure f
...
contractUsedMany : {remove : _} ->
                   (Used (remove +%+ vars)) ->
                   Used vars
contractUsedMany {remove=SLNil} x = x
...
mutual
  ...
  makeLam {vars} fc bound sc
    ...
    where
        ...
        allPrfs : (vs : ScopedList Name) -> (unused : Vect (length vs) Bool) -> ScopedList (Var vs)
        allPrfs SLNil _ = SLNil
        ...
        allVars : FC -> (vs : ScopedList Name) -> (unused : Vect (length vs) Bool) -> ScopedList (Lifted vs)
...
-- otherwise use old behaviour (thunk is a function)
  ...
  liftExp (CRef fc n) = pure $ LAppName fc lazy n SLNil -- probably shouldn't happen!
  liftExp (CLam fc x sc) = makeLam {doLazyAnnots} {lazy} fc (x :%: SLNil) sc
  ...
  liftExp (CForce fc lazy tm) = if doLazyAnnots
    ...
    else liftExp {doLazyAnnots} (CApp fc tm (CErased fc :%: SLNil))
  ...
  usedVars used (LLet fc x val sc) =
    let innerUsed = contractUsed $ usedVars (weakenUsed {outer=(x :%: SLNil)} used) sc in
  ...
  dropIdx SLNil (False::_) First = MkVar First
  dropIdx SLNil (True::_) First = assert_total $
    idris_crash "INTERNAL ERROR: Referenced variable marked as unused"
  dropIdx SLNil (False::rest) (Later p) = Var.later $ dropIdx SLNil rest p
  dropIdx SLNil (True::rest) (Later p) = dropIdx SLNil rest p
...
lambdaLiftDef doLazyAnnots n (MkFun args exp)
    = do (expl, defs) <- liftBody {doLazyAnnots} n exp
         pure ((n, MkLFun args SLNil expl) :: defs)
