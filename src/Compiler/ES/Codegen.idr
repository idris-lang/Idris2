...
-- A name from the preamble (file `support/js/runtime.js`).
...
data SupportMode
  = SupportMode_NoFunction_NoFilename
  | SupportMode_NoFunction_Filename String
  | SupportMode_Function_Filename String String

breakMaybeFunctionAndfileNameWithoutExtension : String -> SupportMode
breakMaybeFunctionAndfileNameWithoutExtension s =
  case breakDrop1 ',' s of
    ("", "") => SupportMode_NoFunction_NoFilename
    (x, "") => SupportMode_NoFunction_Filename x
    (x, y) => SupportMode_Function_Filename x y
...
makeForeign idrisFunctionWithNamespace foreignFunctionBackend foreignFunctionTypeAndCode = do
  noMangleMap <- get NoMangleMap
  varFromName <- getOrRegisterRef idrisFunctionWithNamespace
  ...
  case readCCPart foreignFunctionTypeAndCode of
    ("support", x) => do
      ...
      jsFunctionNameString_fromJsFile <- case idrisFunctionName of
        UN (Basic jsFunctionNameString) => pure jsFunctionNameString
        other => errorConcat ["Invalid foreign type for function \{show idrisFunctionWithNamespace}: the function name should be Basic, but got \{show other}"]
      let jsFunctionNameString_importedAs = jsMangleName idrisFunctionWithNamespace
      case breakMaybeFunctionAndfileNameWithoutExtension x of
        SupportMode_NoFunction_NoFilename => do
          let fileNameWithoutExtension = Core.Name.Namespace.showNSWithSep "." idrisFunctionNamespace
          addToImportFileToPreamble (foreignFunctionBackend, fileNameWithoutExtension) (jsFunctionNameString_fromJsFile, jsFunctionNameString_importedAs)
        SupportMode_NoFunction_Filename fileNameWithoutExtension =>
          addToImportFileToPreamble (foreignFunctionBackend, fileNameWithoutExtension) (jsFunctionNameString_fromJsFile, jsFunctionNameString_importedAs)
        SupportMode_Function_Filename customFunctionName fileNameWithoutExtension =>
          addToImportFileToPreamble (foreignFunctionBackend, fileNameWithoutExtension) (customFunctionName, jsFunctionNameString_importedAs)
      pure Nothing
  ...
  (UN (Basic "prim__os"), []) => do
    case !(supportedBackendPrimaryAndFallback <$> get ESs) of
         NodePreferredJavascriptFallback => do
             let (idrisFunctionNamespace, idrisFunctionName) = Core.Name.splitNS nm
             let fileBaseName = Core.Name.Namespace.showNSWithSep "." idrisFunctionNamespace
             let jsFunctionNameString_fromJsFile = "prim__os"
             let jsFunctionNameString_importedAs = jsMangleName nm
             addToImportFileToPreamble (EsForeignBackend_Node, fileBaseName) (jsFunctionNameString_fromJsFile, jsFunctionNameString_importedAs)
             pure $ Text $ jsFunctionNameString_importedAs
...
------------ Printers
printFilePath_without_ext : (EsForeignBackend {- dir -}, String {- file -}) -> String
printFilePath_without_ext (backend, fileName) = esForeignBackend__toDir backend ++ "/" ++ fileName

printFilePath_with_ext : (EsForeignBackend {- dir -}, String {- file -}) -> String
printFilePath_with_ext (backend, fileName) = printFilePath_without_ext (backend, fileName) ++ ".js"

printNameImport : (String {- js func name -}, String {- import as -}) -> String
printNameImport (jsFunctionNameString_fromJsFile, jsFunctionNameString_importedAs) =
  "  \{jsFunctionNameString_fromJsFile} as \{jsFunctionNameString_importedAs}"

printImport : ((EsForeignBackend {- dir -}, String {- file -}), List (String {- js func name -}, String {- import as -})) -> String
printImport ((backend, fileName), names) =
  "import {\n\{concat $ intersperse ",\n" $ map printNameImport names}\n} from \"./\{printFilePath_with_ext (backend, fileName)}\";"

-- will print
-- import { foo as My_Module_foo } from "./node/My.Module.js"
public export
printPreamble : List ((EsForeignBackend {- dir -}, String {- file -}), (List (String {- js func name -}, String {- import as -}))) -> String
printPreamble preamble = concat $ intersperse "\n" $ map printImport preamble
...
compileToES c s cg tm esSupportedBackend outputDir = do
  ...
  let runtimeForeign = "import * as $runtime from \"./js/runtime.js\";\nimport { __tailRec } from \"./js/runtime.js\";"
  ...
  -- complete preamble, including content from additional support files (if any)
  let preamble' : List ((EsForeignBackend, String), List (String, String)) = map (\(k, v) => (k, Prelude.toList v)) $ Libraries.Data.SortedMap.toList $ preamble st

  for_ supportedBackendListString $ \backend => do
    let dest = outputDir </> backend
    Right ok <- coreLift $ Directory.Tree.removeDirRecursive (parse $ dest)
      | Left err => errorConcat $ ["copyPreludeFile -> removeDirRecursive failed for \{show backend}: ", show err]
    pure ok

  let importedJavascriptFiles = map printFilePath_with_ext $ map fst preamble'

  for_ (["js/runtime.js"] ++ importedJavascriptFiles) $ \dataFilename => do
    let dest = outputDir </> dataFilename
    -- coreLift_ $ (printLn $ "trying: \{show dest}")
    fromPath <- findDataFile dataFilename
    whenJust (parent dest) ensureDirectoryExists
    Right ok <- coreLift $ copyFile fromPath dest
      | Left err => errorConcat $ ["copyPreludeFile -> cannot copy \{show fromPath} to \{show $ dest}: ", show err]
    pure ok

  pure $ fastUnlines [printPreamble preamble', allDecls, main]
