...
-- binding outside
tryLiftOut : (new : Name) ->
             ScopedList (CConAlt vars) ->
             Maybe (ScopedList (CConAlt (new :%: vars)))
tryLiftOut new SLNil = Just SLNil
...
tryLiftOutConst : (new : Name) ->
                  ScopedList (CConstAlt vars) ->
                  Maybe (ScopedList (CConstAlt (new :%: vars)))
tryLiftOutConst new SLNil = Just SLNil
...
allLams : ScopedList (CConAlt vars) -> Bool
allLams SLNil = True
allLams (MkConAlt n ci t args (CLam _ _ _) :%: as)
   = allLams as
allLams _ = False

allLamsConst : ScopedList (CConstAlt vars) -> Bool
allLamsConst SLNil = True
...
doCaseOfCase : FC ->
               (x : CExp vars) ->
               (xalts : ScopedList (CConAlt vars)) ->
               (xdef : Maybe (CExp vars)) ->
               (alts : ScopedList (CConAlt vars)) ->
               (def : Maybe (CExp vars)) ->
               CExp vars
...
doCaseOfConstCase : FC ->
                    (x : CExp vars) ->
                    (xalts : ScopedList (CConstAlt vars)) ->
                    (xdef : Maybe (CExp vars)) ->
                    (alts : ScopedList (CConstAlt vars)) ->
                    (def : Maybe (CExp vars)) ->
                    CExp vars
...
tryCaseOfCase (CConCase fc (CConCase fc' x xalts xdef) alts def)
  ...
  where
    ...
    canCaseOfCase : ScopedList (CConAlt vars) -> Maybe (CExp vars) -> Bool
    canCaseOfCase SLNil _ = True
    canCaseOfCase (x :%: SLNil) Nothing = True
...
tryCaseOfCase (CConstCase fc (CConstCase fc' x xalts xdef) alts def)
  ...
  where
    ...
    canCaseOfCase : ScopedList (CConstAlt vars) -> Maybe (CExp vars) -> Bool
    canCaseOfCase SLNil _ = True
    canCaseOfCase (x :%: SLNil) Nothing = True
