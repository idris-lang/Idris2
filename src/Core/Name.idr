...
data UserName : Type where
  ...
  Hole  : String -> UserName -- user-named hole in the program e.g. ?hole
...
Ord UserName where
  ...
  compare (Hole x) (Hole y) = compare x y
...
userNameEq (Hole x) (Hole y) with (decEq x y)
  userNameEq (Hole x) (Hole y) | Yes prf = Just (cong Hole prf)
  userNameEq (Hole x) (Hole y) | No nprf = Nothing
