...
data Term : Scoped where
     ...
     Meta : FC -> Name -> Int -> ScopedList (Term vars) -> Term vars
     Bind : FC -> (x : Name) ->
            (b : Binder (Term vars)) ->
...
mutual
  ...
  export
  shrinkTerms : Shrinkable (ScopedList . Term)
  shrinkTerms ts th
    = assert_total
...
mutual
  ...
  export
  thinTerms : Thinnable (ScopedList . Term)
...
mutual
  ...
  resolveNamesTerms : (vars : ScopedList Name) -> ScopedList (Term vars) -> ScopedList (Term vars)
