...
mutual
  ...
  public export
  Spine : Scoped
  Spine vars = SnocList (FC, Closure vars)
  ...
  data NHead : Scoped where
       ...
       NMeta  : Name -> Int -> Spine vars -> NHead vars
...
export
covering
[CLOSURE] {free : _} -> Show (Closure free) where
  show (MkClosure opts lenv rho t) = show t
  show (MkNFClosure x rho nf) = show nf

export
covering
[LOCALENV] {free : _} -> Show (LocalEnv free vars) where
  show locs = #"[<\#{joinBy ", " (cast $ showAll locs)}]"# where

    showAll : LocalEnv free vars' -> SnocList String
    showAll [<] = [<]
    showAll (locs :< cl) = showAll locs :< show @{CLOSURE} cl
