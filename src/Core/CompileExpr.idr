...
mutual
  ...
  data CExp : ScopedList Name -> Type where
       ...
       CApp : FC -> CExp vars -> ScopedList (CExp vars) -> CExp vars
       ...
       CCon : FC -> Name -> ConInfo -> (tag : Maybe Int) ->
              ScopedList (CExp vars) -> CExp vars
       ...
       CExtPrim : FC -> (p : Name) -> ScopedList (CExp vars) -> CExp vars
       ...
       CConCase : FC -> (sc : CExp vars) -> ScopedList (CConAlt vars) -> Maybe (CExp vars) -> CExp vars
       CConstCase : FC -> (sc : CExp vars) -> ScopedList (CConstAlt vars) -> Maybe (CExp vars) -> CExp vars
       -- A primitive value
...
mutual
  ...
  data NamedCExp : Type where
       ...
       -- exactly the same length as expected by its definition (so saturate with
       -- lambdas if necessary, or overapply with additional CApps)
       NmApp : FC -> NamedCExp -> ScopedList NamedCExp -> NamedCExp
       -- A saturated constructor application
       -- 'ConInfo' gives additional information about the constructor that
       -- a back end might be able to use. It is ignoreable.
       NmCon : FC -> Name -> ConInfo -> (tag : Maybe Int) ->
               ScopedList NamedCExp -> NamedCExp
       -- Internally defined primitive operations
       NmOp : {arity : _ } -> FC -> PrimFn arity -> Vect arity NamedCExp -> NamedCExp
       -- Externally defined primitive operations
       NmExtPrim : FC -> (p : Name) -> ScopedList NamedCExp -> NamedCExp
       -- A forced (evaluated) value
       NmForce : FC -> LazyReason -> NamedCExp -> NamedCExp
       -- A delayed value
       NmDelay : FC -> LazyReason -> NamedCExp -> NamedCExp
       -- A case match statement
       NmConCase : FC -> (sc : NamedCExp) -> ScopedList NamedConAlt -> Maybe NamedCExp -> NamedCExp
       NmConstCase : FC -> (sc : NamedCExp) -> ScopedList NamedConstAlt -> Maybe NamedCExp -> NamedCExp
       -- A primitive value
...
data CFType : Type where
     ...
     CFUser : Name -> ScopedList CFType -> CFType
