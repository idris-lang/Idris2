...
parameters (defs : Defs, topopts : EvalOpts)
  mutual
    ...
    eval {vars} {free} env locs (Meta fc name idx args) stk
        = evalMeta env fc name idx (closeArgs (cast args)) stk
      where
        ...
        closeArgs : SnocList (Term (free ++ vars)) -> Spine free
        closeArgs = map ((emptyFC,) . MkClosure topopts locs env)
    ...
    eval env locs (App fc fn arg) stk
        = do arg <- case strategy topopts of
               CBV => MkNFClosure topopts env <$> eval env locs arg []
               CBN => pure $ MkClosure topopts locs env arg
             log "eval.app" 50 $ "Pushing (closure \{show @{CLOSURE} arg}) on the stack"
    ...
    -- TODO: this is just a vector update
    -- We should refactor LocalEnv
    ...
    evalTree env loc opts fc stk (Case {name} idx x _ alts)
      = do xval <- evalLocal env fc Nothing idx (embed x) [] loc
           ...
           logC "eval.casetree" 5 $ do
             ...
             pure "In \{show @{LOCALENV} loc}, evaluated \{show name} to \{show xval}"
           let loc' = updateLocal opts env idx (embed x) loc xval
